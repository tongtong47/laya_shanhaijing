...
/** 海经表行数据接口 */
export interface ISeaRow {
    /** 章节编号 */
    id: number;
    /** 章节标识名（如 seas_chapter_1） */
    name: string;
    /** 解锁状态：0 = 已解锁，1 = 锁定 */
    lock: number;
    /** 配图资源名，"&" 分隔 */
...
export class SeaData {
    /** 海经表：id -> 行数据 */
    private _table: Map<number, ISeaRow> = new Map();
    /** 是否已成功解析 */
    ...
    public parse(raw: any[] | string): void {
        ...
        if (typeof raw === "string" && raw.length > 0) {
            try {
                ...
                console.error("解析海经字典 JSON 失败：", e);
                return;
            }
        }
        ...
        if (data && Array.isArray(data)) {
            for (const item of data) {
                ...
                this._table.set(row.id, row as ISeaRow);
            }
    ...
    public getRow(id: number): ISeaRow | null {
        return this._table.get(id) || null;
    }

    /** 取全部章节行（按 id 升序） */
    public getAll(): ISeaRow[] {
