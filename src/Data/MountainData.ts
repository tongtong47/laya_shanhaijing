...
export class MountainData {
    /** 山经表：id -> 行数据 */
    private _table: Map<number, IMountainRow> = new Map();
    /** 是否已成功解析 */
    ...
    public parse(raw: any[] | string): void {
        ...
        if (typeof raw === "string" && raw.length > 0) {
            try {
                ...
                console.error("解析山经字典 JSON 失败：", e);
                return;
            }
        }
        ...
        if (data && Array.isArray(data)) {
            for (const item of data) {
                ...
                this._table.set(row.id, row as IMountainRow);
            }
    ...
    public getRow(id: number): IMountainRow | null {
        return this._table.get(id) || null;
    }

    /** 取全部章节行（按 id 升序） */
    public getAll(): IMountainRow[] {
